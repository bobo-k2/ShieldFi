...
export async function analyticsRoutes(app: FastifyInstance) {
  ...
  app.get('/api/stats', async (_req, reply) => {
    ...
    const [totalScans, totalWallets, scanLast24h, activeSubscriptions, monitoredWallets, telegramSubscribers] = await Promise.all([
    ...
    const data = { totalScans, totalWallets, scanLast24h, activeSubscriptions, monitoredWallets, telegramSubscribers };
